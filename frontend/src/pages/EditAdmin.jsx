...
function EditAdmin() {
    ...
    const [formData, setFormData] = useState({
        name: adminFromState?.name || '',
        email: adminFromState?.email || '',
        password: '',
        role: adminFromState?.role || 'admin'
    })
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (!res.ok) {
                ...
                throw new Error(data.message || 'حدث خطأ أثناء تعديل المسؤول')
            }

            navigate('/admins')
        } catch (err) {
            setError(err.message || 'حدث خطأ أثناء تعديل المسؤول')
        } finally {
            setLoading(false)
        }
    }
            ...
            >
                ...
                <div className="p-5">
                    ...
                    <div className="text-center mb-4">
                        ...
                        <h1 className="text-lg font-bold gold-text">تعديل المسؤول</h1>
                    </div>
                    ...
                    <form onSubmit={handleSubmit} className="space-y-3">
                        ...
                        {/* Password Input */}
                        <div>
                            <label className="block text-white/60 text-xs mb-1 text-right">كلمة المرور الجديدة (اختياري)</label>
                            <div className="relative">
                                <button
                                    type="button"
